...
def insert_a_favourite_person(request):
    ...
    # @Shaziah
    # Make copy of FP audio clip and move it into the Voice Clone module
    shutil.copy2(os.getcwd() + r"/tmp/" +"recording_1.wav", os.getcwd() + r"/voice_clone/Real_Time_Voice_Cloning/audio_clips/" +"FP_audio_clip.wav")       
    ...
    # Now that FP is created, we can get the FP ID generated by SQLite needed to upload the FP clips
    FP_id = cur.lastrowid

    exec(open(r"voice_clone/Real_Time_Voice_Cloning/demo_cli.py").read())
    
    # Pad all clips with silence
    # days of week: 2.5 seconds, months: 2.5 seconds, numbersth: 2.5, 2023: 3
    # Then concatenate all short clips (days of week, months, numbers, year 2023)
    ...
    for clip in short_clips[:50]: # 7 days of the week + 12 months of the year + 31 numbers = 50 clips
        ...
        if duration < 2500: 
            # Pad the audio file with silence
            silence_segment = AudioSegment.silent(duration=(2500-duration))
            wav_file = wav_file + silence_segment

        elif duration > 2500:
            # audio file is too long, so crop it
            wav_file = wav_file[:(2500)]
        
        # if this is the first file
        if combined_clips is None:
            combined_clips = wav_file
        
        # add the new file to the current combined file in progress
        else:
            combined_clips = combined_clips + wav_file
        
    # Repeat the above for the year, 2023
    clip = short_clips[-1]
    filename = os.getcwd() + r"/voice_clone/Real_Time_Voice_Cloning/audio_clips/output/" + clip + ".wav"
    wav_file = AudioSegment.from_file(file=filename, format="wav")
    duration = len(wav_file)
    if duration < 3000: 
        # Pad the audio file with silence
        silence_segment = AudioSegment.silent(duration=(3000-duration))
        wav_file = wav_file + silence_segment

    elif duration > 3000:
        # audio file is too long, so crop it
        wav_file = wav_file[:(3000)]
