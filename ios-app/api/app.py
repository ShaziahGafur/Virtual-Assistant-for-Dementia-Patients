...
from voice_clone.Real_Time_Voice_Cloning.demo_cli import text_to_speech_voice_cloner
...
def insert_a_favourite_person(request):
    ...
    mp3files = [f for f in listdir(mp3path) if isfile(join(mp3path, f))]
    ...
    for clip in mp3files:
        blob = bucket.blob(audio_folder + clip) 
        blob.upload_from_filename(mp3path + clip)
