...
export default function RecordAudio() {
  ...
  async function playRecording() {
    ...
    const { sound } = await Audio.Sound.createAsync({
      uri: recording.getURI() || URIFROMFileSystem,
    });
  ...
  async function playSampleSound() {
    ...
    const { sound } = await Audio.Sound.createAsync(
      require("./assets/sounds/example.mp3")
    );
    setSound(sound);
    setSoundIsPlaying(true);
  ...
  return (
    <View style={styles.container}>
      ...
      {recording && (
        <Button title={"Play Recording"} onPress={playRecording}></Button>
      )}

      {soundIsPlaying && (
        <Button title={"Stop Playing"} onPress={stopPlayingRecording}></Button>
      )}
      <Button
        title={"Get Sample Transcription"}
        onPress={getSampleAudioTranscription}
      ></Button>
      <Button
        title={"Get Recording Transcription"}
        onPress={getRecordingTranscription}
      ></Button>
